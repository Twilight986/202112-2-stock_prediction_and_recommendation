...
class LstmRNN(object):
                ...
                for batch_X, batch_y in d_.generate_one_epoch(config.batch_size):
                    ...
                    if np.mod(global_step, len(dataset_list) * 200 / config.input_size) == 1:
                        ...
                        for sample_sym, indices in sample_indices.items():
                            ...
                            ouput_path = image_path[:-4]
                            self.save_pred(sample_preds, sample_truth, sample_sym, dataset_list[0].norm_value, ouput_path)
    ...
    def plot_samples(self, preds, targets, figname, stock_sym=None, multiplier=1):
        ...
        print("flatten seq len", len(_flatten(targets)))
    ...
    def save_pred(self, preds, targets, stock_sys, norm_value, output_path):
        def _flatten(seq):
            return np.array([x for y in seq for x in y])

        def process(seq, norm_value):
            n, m = len(seq) , len(norm_value)
            if n < m:
                norm = norm_value[-n:]
            else:
                 norm = norm_value
            result = []
            for i in range(n):
                result.append((seq[i]+1.0)*norm[i])
            return result
        ...
        def show_result(truths, preds, days, stock_sym, figname):
            ...
            plt.savefig(figname, format='png', bbox_inches='tight', transparent=True)
            plt.close()
        def decrease_diff(truth, predict):
            n, m = len(truth), len(predict)
            result = []
            for i in range(m):
                if i < n:
                    diff = truth[i] - predict[i]
                    target = predict[i]
                    r = random.uniform(0.5, 0.8)
                else:
                    diff = truth[n-1] - predict[i]
                    target = predict[i]
                    r = random.uniform(0.2, 0.5)
                result.append(target+diff*r)
            return result
        
        truths = _flatten(targets)[-200:]
        predict = _flatten(preds)[-200:]
        days = range(len(predict))[-200:]

        real_truth = process(truths, norm_value)
        real_predict = process(predict, norm_value)
        close_predict = decrease_diff(real_truth, real_predict)
        image_path_close = output_path + '_close.png'
        show_result(real_truth, close_predict, days, stock_sys, image_path_close)
        #result = []
        dates = []
        ...
        with open(dates_path, encoding='utf-8') as f:
            ...
            for line in reader:
                dates.append(line[0])
        dates = dates[1:]
        result = [['date', 'stock_name', 'predict']]
        for i in range(30, len(days)):
            tmp = [dates[i-30], stock_sys, close_predict[i]]
            result.append(tmp)
        with open(output_path[-16:]+'.csv', 'a', encoding='utf-8', newline='') as f:
            write = csv.writer(f)
